...
Datasets comparision.Get insights from data.
...
#def pairwise_report(df1,df2,target=None,ignore=[],maxcount=20,full=True):
    # Compare two datasets:
    # df1      pandas dataframe     first dataset 
    # df2      pandas dataframe     second dataset
    # ignore  list                  features to ignore (optional)
    # maxcount  int                   max number of top features (with max shap values) to print

#creates html report
def pairwise_report(df1,df2,target=None,ignore=[],maxcount=None,full=True,maxcount20=20):
    """Dataset comparison.
    ...
    maxcount=maxcount if maxcount!=None else max(df1.shape[1],df2.shape[1])
    ...
    if target==None:#compare datasets
        sorted_features=plot_shap_pairwise(df1,df2,ignore=ignore,maxcount=maxcount)
    ...
    sorted_features=sorted_features[:maxcount]
    ...
    for feature in features:
        ...
        if info1.iloc[0,0]=='Categorical' or info1.iloc[0,0]=='Boolean':
            ...
            if info1.iloc[1,0]<=25:
                ...
                fig,(ax1, ax2) = pls.subplots(ncols=2,figsize=(16, 5))
...
def plot_shap_pairwise(df1, df2,ignore=[],maxcount=20):
    """
    """    
    ...
    return plot_shaps(x,target,ignore=ignore,maxcount=maxcount)
...
def print_na_pairwise(df1,df2,max_row=60):
    display_side_by_side([missing_values_table(df1).head(max_row),missing_values_table(df2).head(max_row)],
                         ['Frame 1 ','Frame 2'])
...
def __plot_pairwise_stats(df_1,df_2,feature,maxcount=30):
...
def plot_pairwise_stats(df1,df2,feature,target,maxcount=30):
    """Plots pairwise feature statistics.
    """    
    if target != None:
        
        fig, axs = pls.subplots(ncols=2,nrows=2,figsize=(16,14))

        if feature in df1.columns and not df1[feature].isnull().values.all():
            plot_stats(df1,feature,target,maxcount=maxcount,sortby='Count ',ax=axs[:,0])
        ...
        __plot_pairwise_stats(df1,df2,feature,maxcount=30)
        

#=====================#=====================#=====================
# time series plots
#=====================#=====================#=====================
def pairwise_feature_stat_per_day(df1,df2,target, **kwarg ):
    ...
    Args:
        ...
        df1,df2   (str): feature for grouping (categorical)
